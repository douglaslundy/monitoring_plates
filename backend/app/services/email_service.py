...
def send_face_alert(
    ...
    conf_html = (
        f'<tr style="background:#f8f8f8;"><td style="padding:10px 12px;font-weight:bold;">Confiança</td>'
        f'<td style="padding:10px 12px;">{round(confidence * 100, 1)}%</td></tr>'
    ...
    html = f"""
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;">
      <h2 style="color:#7c3aed;margin-top:0;">&#128100; Alerta de Reconhecimento Facial</h2>
      <table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
        <tr style="background:#f8f8f8;">
          <td style="padding:10px 12px;font-weight:bold;width:30%;">Pessoa</td>
          <td style="padding:10px 12px;font-size:18px;font-weight:bold;">{person_name}</td>
        </tr>
        <tr>
          <td style="padding:10px 12px;font-weight:bold;">C&#226;mera</td>
          <td style="padding:10px 12px;">{camera_name}</td>
      ...
      {image_html}
      <p style="margin-top:24px;">
        <a href="http://localhost:3000" style="background:#7c3aed;color:#fff;padding:10px 20px;
           text-decoration:none;border-radius:4px;">Acessar o Sistema</a>
      </p>
    </div>
    """
        ...
        resend.Emails.send(
            {
                "from": settings.RESEND_FROM_EMAIL,
                "to": [to],
                "subject": f"\U0001f464 {person_name} reconhecido(a) — {camera_name}",
                "html": html,
            }
        )
        return True
    except Exception:
        logger.warning("Failed to send face alert email to %s", to, exc_info=True)
        return False
